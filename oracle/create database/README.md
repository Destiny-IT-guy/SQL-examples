...
### 2. Vachar2
this data type allow us to a string a variable length string.
example 
```sql
Name varchar2(15), Not Null.
```
this create a column named "name" which i can enter a string of maximum length 15 but it cant be null so it has to have a minimum of 1 character string. Also after you select a varchar2 number it is possible to increase the number but impossible to decrease the number so i can increase it to 20 but cant decrease it to 10.

### date
this data type store data in a date type format like YYYY-MM-DD or DD-MM-YYYY and many more unconventional type
example
```sql
...
to link multiple different table together we need to use  keys. first there is the primary keys. they 

### primary key
Primary key uniquely identifies each row in a table.  
It ensures that no two rows have the same value in the primary key column(s) and that the value cannot be NULL. It is recommended to at least have 1 primary key for each table

### foreign key
When i want to connect 2 table i would usually add the column of the primary key of the first table to the second table. 
...
## adding foreign key
adding a foreign key is a lil more complex than a primary key. to start after adding each column in you table you have to add the following to lines
```sql
...
```
I created 3 table which table 1 and 2 are linked to table 3 but they aren't directly link to each other 

to delete a table its pretty simple just enter the following command
```sql
...
## end
My next part i will show myself how to create a bigger database and add data and search for data.